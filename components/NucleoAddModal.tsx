...
import { Nucleo, NucleoTurma, ProjectId } from '../types';
...
export const NucleoAddModal: React.FC<NucleoAddModalProps> = ({ isOpen, onClose, onSave, projectId = 'FORMANDO_CAMPEOES' }) => {
    const tc = useMemo(() => {
        if (projectId === 'FUTEBOL') return { gradient: 'from-green-500 to-emerald-600', sub: 'text-green-100', ring: 'focus:ring-green-400', badge: 'bg-green-100 text-green-600', badgeBorder: 'border-green-200', dayOn: 'bg-green-600 text-white border-green-600', dayOff: 'hover:border-green-300', durBg: 'bg-green-50 border-green-100', durIcon: 'text-green-500', durText: 'text-green-700', addBtn: 'text-green-600 hover:text-green-800 bg-green-50 hover:bg-green-100 border-green-200', submit: 'from-green-500 to-emerald-600 shadow-green-200 hover:shadow-emerald-300 hover:from-green-600 hover:to-emerald-700' };
        if (projectId === 'INCENTIV_APP') return { gradient: 'from-indigo-600 to-blue-600', sub: 'text-indigo-100', ring: 'focus:ring-indigo-400', badge: 'bg-indigo-100 text-indigo-600', badgeBorder: 'border-indigo-200', dayOn: 'bg-indigo-600 text-white border-indigo-600', dayOff: 'hover:border-indigo-300', durBg: 'bg-indigo-50 border-indigo-100', durIcon: 'text-indigo-500', durText: 'text-indigo-700', addBtn: 'text-indigo-600 hover:text-indigo-800 bg-indigo-50 hover:bg-indigo-100 border-indigo-200', submit: 'from-indigo-600 to-blue-600 shadow-indigo-200 hover:shadow-blue-300 hover:from-indigo-700 hover:to-blue-700' };
        if (projectId === 'DANIEL_DIAS') return { gradient: 'from-sky-500 to-slate-500', sub: 'text-sky-100', ring: 'focus:ring-sky-400', badge: 'bg-sky-100 text-sky-600', badgeBorder: 'border-sky-200', dayOn: 'bg-sky-600 text-white border-sky-600', dayOff: 'hover:border-sky-300', durBg: 'bg-sky-50 border-sky-100', durIcon: 'text-sky-500', durText: 'text-sky-700', addBtn: 'text-sky-600 hover:text-sky-800 bg-sky-50 hover:bg-sky-100 border-sky-200', submit: 'from-sky-500 to-slate-500 shadow-sky-200 hover:shadow-slate-300 hover:from-sky-600 hover:to-slate-600' };
        return { gradient: 'from-blue-600 to-teal-500', sub: 'text-blue-100', ring: 'focus:ring-blue-400', badge: 'bg-blue-100 text-blue-600', badgeBorder: 'border-blue-200', dayOn: 'bg-blue-600 text-white border-blue-600', dayOff: 'hover:border-blue-300', durBg: 'bg-blue-50 border-blue-100', durIcon: 'text-blue-500', durText: 'text-blue-700', addBtn: 'text-blue-600 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 border-blue-200', submit: 'from-blue-600 to-teal-500 shadow-blue-200 hover:shadow-teal-300 hover:from-blue-700 hover:to-teal-600' };
    }, [projectId]);
    ...
    return (
        <div className="fixed inset-0 z-[70] flex items-start justify-center bg-black/50 backdrop-blur-sm overflow-y-auto p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl my-8 overflow-hidden">
                {/* Header */}
                <div className={`bg-gradient-to-r ${tc.gradient} px-6 py-5 flex items-center justify-between`}>
                    <div>
                        <h2 className="text-white font-bold text-lg">Novo Núcleo</h2>
                        <p className={`${tc.sub} text-sm mt-0.5`}>Preencha as informações do núcleo</p>
                    </div>
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div>
                        <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3 flex items-center gap-2">
                            <span className={`w-5 h-5 ${tc.badge} rounded-full flex items-center justify-center text-[10px] font-black`}>1</span>
                            Identificação
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="md:col-span-2">
                                ...
                                <input
                                    ...
                                    className={`w-full border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 ${tc.ring} ${errors.nome ? 'border-red-400 bg-red-50' : 'border-gray-200 bg-gray-50'}`}
                                />
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-600 mb-1">N° SLI</label>
                                <input
                                    type="text"
                                    value={sliNumber}
                                    onChange={e => setSliNumber(e.target.value)}
                        ...
                        <div className="bg-gray-50 rounded-xl border border-gray-200 p-4 space-y-4">
                                    ...
                                    {DIAS_SEMANA.map(dia => (
                                        <button
                                            ...
                                            className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-colors ${dias.includes(dia)
                                                    ? tc.dayOn
                                                    : `bg-white text-gray-500 border-gray-200 ${tc.dayOff}`
                                                }`}
                                        >
                                            {dia.substring(0, 3)}
                                        </button>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className={`w-full border border-gray-200 bg-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 ${tc.ring}`}
                                />
                            </div>
                            ...
                            {durabilidade && (
                                <div className={`${tc.durBg} rounded-lg px-3 py-2 flex items-center gap-2 border`}>
                                    ...
                                    <span className={`text-xs font-bold ${tc.durText}`}>Durabilidade: {durabilidade}</span>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <div className="border-t border-gray-100 px-6 py-4 bg-gray-50 flex gap-3">
                    ...
                    <button
                        ...
                        className={`flex-1 py-2.5 bg-gradient-to-r ${tc.submit} text-white font-bold rounded-xl shadow-lg text-sm transition-all`}
                    >
                        ✓ Criar Núcleo
                    </button>
                </div>
