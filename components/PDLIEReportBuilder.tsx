...
import { Nucleo, EvidenceLog, SliGroup } from '../types';
import { ReportEditorToolbar } from './ReportEditorToolbar';
import { SliNucleoSelect } from './SliNucleoSelect';

interface Props { nucleos: Nucleo[]; evidences: EvidenceLog[]; onBack: () => void; headerImage?: string; sliGroups?: SliGroup[]; }
...
export const PDLIEReportBuilder: React.FC<Props> = ({ nucleos, evidences, onBack, headerImage = '/header_full.png', sliGroups = [] }) => {
      ...
      <div ref={reportRef} className="freq-report-content">
        ...
        <div className={PS} style={{ padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column', fontFamily: "'Times New Roman', Times, serif", fontSize: 13, lineHeight: 1.8, color: '#222' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', padding: '12px 20mm 8px' }}>
            ...
            <div style={{ background: '#4caf50', color: '#fff', padding: '7px 16px', fontSize: 13, fontWeight: 800, textAlign: 'right', borderRadius: 2, lineHeight: 1.4 }}>
              {city} – {uf}<br/>{yr}
            </div>
          </div>
          {/* Footer text */}
